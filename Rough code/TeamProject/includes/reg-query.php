...
function create_user(int $user_id, string $first_n, string $last_n, string $email, string $pwd){
    global $con;
    $pwd_h= password_hash($pwd, PASSWORD_DEFAULT); //hash password for security

    $update_query = "UPDATE employee SET user_id = $user_id, password = ? , is_reg = 'Y'
     WHERE first_name = ? AND last_name = ? AND email = ?"; //sql statemnt

    //preparing, binding and executing statement
    $update_stmt = $con->prepare($update_query);
    $update_stmt->bind_param("ssss",$pwd_h, $first_n, $last_n, $email);
    ...
    if ($job_role !== null) {
        switch ($job_role) {
            case 'admin':
                ...
                break;
            case 'team_leader':
                header("Location: ../p_dash.php"); //TEMPORARY
                break;
            case 'employee':
