...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    try {
        ...
        if (get_teamleader_count_project($project_id) > 0){
            $errors[] = "A Project leader is already assigned to the selected project.";
        }

        if (get_admin_count_project($project_id) > 0){
            $errors[] = "A Project leader is already assigned to the selected project.";
        }
        ...
        if(assignment_count($employee_id, $project_id) > 0){
            $errors[] = "Employee is already assigned to the selected project(s).";
        }
